  ...
  <div className="overflow-x-auto shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
    <table className="min-w-full divide-y divide-gray-200">
      <thead className="bg-gray-50">
        <tr>
          {["Img", "Nombre", "Ingredientes", "Precio", "Categoría", "Variantes", "Acciones"].map((header) => (
            <th
              key={header}
      ...
      <tbody className="bg-white divide-y divide-gray-200">
        {items.map((item) => (
          <tr key={item.id}>
            ...
            <td className="px-6 py-4 text-sm text-gray-900">${item.precio.toFixed(2)}</td>
            <td className="px-6 py-4 text-sm text-gray-500">{item.categoria.nombre}</td>
            <td className="px-6 py-4 text-sm text-gray-500">{item.variantes?.length}</td>
